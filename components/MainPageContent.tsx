...
import {WeatherForecastResponseTypes, HourWeatherType} from "../pages/main";
...
const MainPageContent = ({data, isActive, index}: PropsType) => {
	...
	const timeWeatherElements = data.forecast.forecastday[0].hour.map((hour: HourWeatherType) => <HourWeatherCard
	...
	return (
		<>
			{isActive && <div className={`${styles.weatherCards} `}>

        <div className={styles.todayWeatherCard}>
          <Link href={`/${data.location.name}/`}>
            <button className={`${styles.button} ${styles.showMore}`}>Show more</button>
          </Link>
