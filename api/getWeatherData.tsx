...
import { City, WeatherForecastResponseTypes } from "../pages/index";
...
export const getWeather = async (): Promise<StateType> => {
  ...
  try {
    const minskWeather = await fetch(
      `${BASE_URL}&q=${City.Minsk}&days=${days}&aqi=no&alerts=no`
    );
    const moscowWeather = await fetch(
      `${BASE_URL}&q=${City.Moscow}&days=${days}&aqi=no&alerts=no`
    );
    const bratislavaWeather = await fetch(
      `${BASE_URL}&q=${City.Bratislava}&days=${days}&aqi=no&alerts=no`
    );
