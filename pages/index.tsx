...
import {getWeather} from '../hooks/getWeatherData';
...
const MainPage = ({data, error}: PropsType) => {
	...
	const citiesElements = data?.map((city: WeatherForecastResponseTypes, index: number) => <MainPageContent
		key={city.location.name}
		isActive={activeCity === city.location.name}
		data={city}
		index={index}/>);

	const buttonsElements = data?.map((city: WeatherResponseTypes, index: number) => <ButtonContainer key={index}
